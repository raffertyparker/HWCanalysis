...
In our random walk model we assume no drift, and only attempt to predict one (half hour) time step onto the future. From Equation \@ref(eq:randomWalk), we get 
\begin{equation}
\hat x_{t+1} = x_t
\end{equation}


```{r randomWalk, out.width='100%', fig.cap="Actual electricity use and random walk model approximation over one day for four households"}
...
The random walk model has a range of standard deviation of residuals between `r round(min(sdResRW), 2)`W and `r round(max(sdResRW), 2)`W, with a mean of `r round(mean(sdResRW[,1]), 2)`W and median of `r round(median(sdResRW[,1]), 2)`W. The model precisely mimics the actual data with a half-hour lag, and therefore scores highly for physical fidelity. Interpretability is also high, as the output can be understood by the simple equation $\hat x_{t+1} = x_t$. The computational time to fit models for all households considered was `r round(as.numeric(RWproc[3]), 2)` milliseconds, and the 27 models took up 11.9MB of hard drive space.

## Linear regression of hot water electricity against lagged other electricity

While linear regression is a useful method of analysing data that has a roughly linear trend, it is generally inadequate when constructed using data that displays cyclic behaviour, such as residential electricity demand.
...
```{r loadSLdata}
load(paste0(dFile, "models/simpleLinear/procTime.Rda"))
load(paste0(dFile, "models/simpleLinear/sdResSL.Rda"))
```
...
This should be compared to Figure \@ref(fig:crosscovariance), which shows the correllation between hot water and not hot water electricity peaking at around 200 minutes lag.
...
This simple linear model has a range of standard deviation of residuals between `r round(min(sdResSL), 2)`W and `r round(max(sdResSL), 2)`W, with a mean of `r mean(sdResSL[,1])`W and median of `r median(sdResSL[,1])`W. 
...
Interpretability is high, as the output can be understood by the simple equation $\hat x_{t+1} = \alpha_0 + \alpha_1 w_t$, where $w_t$ is non hot water electricity at time $t$. The computational time to fit models for all households considered was `r round(as.numeric(SLproc[3]), 2)` milliseconds, and the 27 models took up 37.4MB of hard drive space.
