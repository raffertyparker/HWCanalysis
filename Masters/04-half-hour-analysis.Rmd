...
The random walk model has a range of standard deviation of residuals between `r round(min(sdResRW), 2)`W and `r round(max(sdResRW), 2)`W, with a mean of `r round(mean(sdResRW[,1]), 2)`W and median of `r round(median(sdResRW[,1]), 2)`W. 
_Standard deviation probably unnecessary, replace with RSS._ 
...
```{r linearRelations, include=FALSE, fig.cap="Linear relation between non hot water electricity and half-hour lagged hot water electricity for household rf_12"}
