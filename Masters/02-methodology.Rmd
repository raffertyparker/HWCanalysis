...
```{r headDT, fig.cap="Example of the clean and processed data used in the analysis"}
knitr::kable(head(DT))
```
...
(\#eq:MA1)
\end{equation}
where $\{w_t\}$ is white noise with zero mean and variance $\sigma^2_w$. Equation \@ref(eq:MA1) can be written in terms of the backshift operator $B$ as
\begin{equation}
...
Using matrix notation, equation \@ref(eq:VAR1) can be written as
\begin{equation}
\bf{Z}_t = \bf{\Theta}\bf{Z}_{t-1} + \bf{w}_t
(\#eq:VAR)
\end{equation}
where $\bf{Z}_t = \begin{pmatrix}x_t \\ y_t \end{pmatrix}$, $\bf{\Theta} = \begin{pmatrix}\theta_{11} & \theta_{12}\\ \theta_{21} & \theta_{22}\end{pmatrix}$, and $\bf{w}_t = \begin{pmatrix}w_{x,t} \\ w_{y,t}\end{pmatrix}$.
...
```{r elementPlot, include=FALSE, out.width='100%', fig.cap="Hot water electricity use over a day for one household"}
