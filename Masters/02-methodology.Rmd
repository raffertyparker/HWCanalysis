...
A time series $x_t$ is an autoregressive process of order $p$ ($p \in \Z $) (referred to as $AR(p)$) if it can be represented as
\begin{equation}
...
More generally, a time series $\{x_t\}$ is referred to as _integrated_ of order $d$ (denoted $I(d)$, where $d \in \Z$) if the $d$th difference of $\{x_t\}$ is white noise.
...
\begin{equation}
AIC = -2 x log-likelihood + 2 x n parameters
(\#eq:AIC)
\end{equation}.

The R function `auto.arima`[CITE] automatically selects the parameters $p,d,q$ that minimise the AIC (with inputs for maximum values necessary as constraints) specific to the particular data being modelled.

As each household has different usage patterns and characteristics, these values differ for each household.
The optimal parameters for each household are provided in the appendix.
_explain how these parameters are automatically selected_
