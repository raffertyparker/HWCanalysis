...
for (house in houses){
  ...
  # Create SARIMA model from training data
  ...
  assign(paste0(house, "_30_min_SARIMA"), 
         auto.arima(ts_fit$HWelec, 
                    stepwise = FALSE, approximation = FALSE, 
                    xreg = ts_fit$dHour)) %>% # Send hour of day as dummy variable
    saveRDS(file = paste0(dFile, "models/SARIMA/", house, "_fitted_model.rds"))
  # Validate model from test data
  assign(paste0(house, "_30_min_SARIMA_validate"), 
         Arima(ts_val$HWelec, 
               xreg = ts_val$dHour, 
               model = get(paste0(house, "_30_min_SARIMA")))) %>%
    saveRDS(file = paste0(dFile, "models/SARIMA/", house, "_validated_model.rds"))
  # Make plot for demonstration purposes
  plotModel(get(paste0(house, "_30_min_SARIMA_validate")), "SARIMA")
}