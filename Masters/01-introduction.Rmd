...
Hot water cylinders under normal usage conditions have a tendency to 'stratify' their contents into distict bodies of water, namely a hot upper layer of water and a cool lower layer of water, separated by a negligible sized, intermediate temperature thermocline[@JackDew2018].
The household residents may use hot water with no noticeable changes in temperature until the hotter top layer suddenly runs out, causing the unpleasant 'cold water event' (CWE), familar to many who live in a household with heavy hot water use.
For households with moderate to low hot water use however, their hot water cylinder may be keeping a far greater amount of water hot than they would expect to use.
...
When working with time series data for the purpose of testing the accuracy of different forecasting methods, it is good practice to separate the dataset into two subests, training and validating, with the training data being a sequential subset of data occurring before the validating data. This helps avoid the problem of 'overfitting', where, in the process of optimising for increased accuracy, forecasting models incorporate anomolies in the data as important variables.
...
[^Data in general can be thought to be made up of dependent and independent (or 'explanatory') variables. Broadly speaking, dependent variables are those we are interested in measuring or predicting (in this research, hot water electricity demand). Explanatory variables are those we expect to impact our dependent variables (in this research, time and the electricity demand of other appliances).]

One conventional forecasting method utilises data of separate but related (explanatory) variables and makes predictions based on basic statistical relationships between them. These are often referred to as _causal_ models[@Hong2016]. Regression analysis is a causal model commonly used in electricity forecasting, and may be understood as follows:
...
This may be through separating the data into individual components, namely trend, cycles and 'noise'. 
...
The weights assigned by each neuron are then iteratively adjusted in order to minimise the error of the output.
...
```{r ANNinDF, fig.cap="Documents in SCOPUS containing keywords: artificial AND neural AND network AND electricity AND demand AND forecasting"}
knitr::include_graphics(paste0(pFile, "artificial_neural_network_electricity_demand_forecasting.png"))
```

This may be due to increasing interest in an alternative AI forecasting method for electricity demand forecasting, that of support vector machines (SVM).
Support vector machines classify datapoints by mapping them in hyperspace and separating them according to which side of a hyperplane they fall on, with the position of the classifying hyperplanes being constructed iteratively in order to maximise the perpendicular distance between each hyperplane and the closest samples on either side of it (refer to Figure \@ref(fig:SVM_example).
Support vector machines may be used in electricity forecasting XXXXXXXXXXXXXXXXXXXXXXXXXx
_up to here_

```{r SVM_example, fig.cap="Example demonstrating the classifying mechanism of a SVM. Image by Larhmam - Own work, CC BY-SA 4.0, https://commons.wikimedia.org/w/index.php?curid=73710028"}
knitr::include_graphics(paste0(pFile, "SVM_margin.png"))
```

This method is far less computationally expensive to train than ANNs, and has no indication of a reduction in research interest.

```{r SVMinDF, fig.cap="Documents in SCOPUS containing keywords: support AND vector AND network AND electricity AND demand AND forecasting"}
...
Other research focused solely on demand response of electric water heaters, usually employing more sophisticated thermal models which allow for greater understanding of potential impacts on service. XXXXXXXXXXXXXXXXXXXXXXXXXX
