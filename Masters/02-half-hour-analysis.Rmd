# Modelling half-hour electricity demand as a continuous output {#half-hour}
<!--
...
```{r NAfix30min}
dtHWlag <- c(tail(dt$HWelec, length(dt$HWelec) - 48*7),
             dt$HWelec[1:(48*7)])
dtNonHWlag <- c(tail(dt$nonHWelec, length(dt$nonHWelec) - 48*7),
                dt$nonHWelec[1:(48*7)])
dt$HWelec[is.na(dt$HWelec) == TRUE] <- dtHWlag
dt$nonHWelec[is.na(dt$nonHWelec) == TRUE] <- dtNonHWlag
# This seems to work ok - maybe not the most elegant method
```
...
On small timescales, the element may then be considered a binary process, impacting our choice of forecasting model. Forecasting a binary output required more sophisticated models than forecasting a "continous" output. 
...
## Simple linear model with lagged HW electricity

Linear regression is a common forecasting model prized for its simplicity in interpretation and computation. A linear regression model fits a straight line through each point within the dataset such that the line minimises the sum of the square of each distance between the line and the data points. _show plot to explain?_
...
_This is a great package that seperates out the cycle, trend, and noise, and plots them. The plotting needs some work as you cannot see the cycles very well (need to 'zoom in' on the plot, which isn't working for me right now)._
...
```{r zoomedIn, fig.cap="Behaviour of data and VAR model over 48 hours"}
...
```{r VARACF, fig.cap="Autocovariance of VAR model"}
...
```{r hourlyAverages, fig.cap="Hourly averages of hot water use"}
...
```{r frequencyPlot, fig.cap="Frequency of power observations"}
...
```{r residualACF, fig.cap="Autocovariance of the VAR model residual"}
...
```{r cyclicalMatrix, include=FALSE}
