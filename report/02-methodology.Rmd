...
The difference between an observed value and the value predicted by the model is formally known as the _residual_, and can be intuitively understood as the error in the value predicted by the model. Most models considered in this analysis aim to have a residual time series that approximates _white noise_, where a white noise time series, $\{w_t : t = 1,2,...,n\}$ is a set of independent and identically distributed variables with zero mean.
...
When long periods of missing data occurred (determined by visual inspection of pleliminary plots) the largest period of uninterrupted data collection was selected for further analysis, with the remainder discarded.
...
While analysis at 1 minute timescales may be beneficial in a future where metering and transmitting infrastructure is capable of facilitating control at this detail, this research attemps to develop methods that could be implimented using existing smart meters.
...
meansDT %>%
  knitr::kable(caption = "Mean values electricity demand for hot water and other appliances")
```

Table \@ref(tab:summary1) shows the mean values of hot water electricitydemand  and other appliance electricity demand for each household. Note the large variation in average demand, which can also be seen in the boxplot shown in Figure \@ref(fig:demandBoxplot).
...
Figure \@ref(fig:averagePlot) shows the average electricity demand over each hour of the day for each household. It can be seen that these demand profiles vary significantly, although many show two distinct peaks, generally during times of day where the grid as a whole experiences peaks. Average demand varies significantly for each household, ranging from `r round(min(meansDT$HWelec), 2)`W (household `r meansDT$linkID[meansDT$HWelec == min(meansDT$HWelec)]`) to `r round(max(meansDT$HWelec), 2)`W (household `r meansDT$linkID[meansDT$HWelec == max(meansDT$HWelec)]`).
...
The results are interpreted by plotting $\gamma_k$ against the lag values. Figure \@ref(fig:autocovariance) indicates that our households are strongly cyclic, with peaks in demand occurring on approximately daily (1440 minute) and 12-hourly (720 minute) intervals, as well as a weekly cycle (peaks increasing again towards the end of the plot). 

### Frequency analysis

_I need to either eleborate on the frequency analysis process or remove the rest of this section. I don't use it in the main analysis any more so perhaps removal is best. Cycles are pretty well covered by STL and ACV._
...
### Cross covarience of hot water electricity use with other appliance electricity use

```{r bothElecPlot, echo=FALSE, out.width='100%', fig.cap="Hot water and other appliances' electricity use over one day"}
knitr::include_graphics(paste0(pFolder, "bothElecPlot.pdf"))
```
Many households appeared to show instances whereby the electricity demand of other appliances increased before the hot water element switched on. This may be understood by considering general domestic behavioural patterns. Many people may choose, for example, to turn on the kettle before a shower in the morning, or cook dinner before doing the dishes in the evening. An example demonstrating this behaviour is given in Figure \@ref(fig:bothElecPlot).
While this figure may not in itself be very compelling, we can make a more thorough exploration into the temporal relationship between hot water electricity demand and that of other appliances using the cross-covariance function.
...
```{r ccvMaxLagPlot, echo=FALSE, out.width='100%', fig.cap="Lag at maximum correllation for each household (bin width 30 minutes)"}
knitr::include_graphics(paste0(pFolder, "maxXcorAllHouses.pdf"))
```

### STL decomposition {#STL}

Another method that may be used to explore any cyclical effects or trends of time series data is that of STL decomposition. STL is an acronym for 'seasonal and trend decomposition using loess', whereby loess (local polynomial regression) is a method for estimating nonlinear relationships. 
An STL decompostition seperates the data into three separate components, referred to as the 'trend', 'seasonality', and 'remainder' (also referred to as 'random'). 
...
The STL method was developed by Cleveland, Cleveland, McRae, & Terpenning (1990)[@Cleveland1990]. As the decomposition algorithm is quite involved it is not included here, but can be examined in the previous reference.
...
A q-order moving average process, $MA(q)$ can be expressed as a linear combination of the current white noise residual ($w_t$, see @TSoverview) and the $q$ most recent previous residuals, defined as
\begin{equation}
...
\end{equation}
where $B$ is the backshift operator, i.e. $Bx_t = x_{t-1}$, and $\phi_q$ is a polynomial of order $q$.
...
The naive model in Section \@ref(naive) is the special case $I(1)$.
...
This method was highly regarded by Gelažanskas and Gamage[@Gelazanskas2015].
...
ARIMAX (ARIMA with external regressor or exogenous variable) models utilise both time series and causal methods by including an explanatory variable (in our case, other appliance electricity demand) as a linear regressor within an ARIMA model.
One way in which this model may be interperated is as a simple linear regression model with ARIMA errors.
This combines Equations \@ref(eq:simpleLinearRegression) and \@ref(eq:ARMA) in the form

\begin{equation}
x_t = \gamma_0 + \gamma_1y_{t-1} + \alpha_1 x_{t-1} + \alpha_2 x_{t-2} + ... + \alpha_p x_{t-p} + w_t + \beta_1 w_{t-1} + \beta_2 w_{t-2} + ... + \beta_q w_{t-q}.
(\#eq:ARMA)
\end{equation}
Where $y$ is the exogenous variable (in our case, other appliance electricity demand).
This may be expressed in terms of the backward shift operator in polynomial form as
\begin{equation}
x_t = \gamma_0 + \gamma_1y_{t-1} + \frac{\theta (\bf{B})}{\phi (\bf{B})} w_t .
(\#eq:ARMAXpolyform)
\end{equation}
...
As demand respose is most crucial during daily peak periods, additional analysis is carried out to acertain the accuracy during grid peaks (defined as 7am to 9am, and 5pm to 8pm, as per reference [@TranspowerNZ2015].
