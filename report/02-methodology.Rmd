...
The effect of this smoothing may be seen in Figure \@ref(fig:elementPlot). 
...
knitr::include_graphics(paste0(pFolder, "elementComparisonPlot.pdf"))
```
...
Columns `hHour`, `min` and `day` are various formats for time which were necessary to separate as model inputs. `linkID` represents the household, and `nonHWelec` and `HWelec` are the half-hour averaged electricity demand of other appliances and hot water, respectively.
...
Table \@ref(tab:summary1) shows the mean values of hot water electricity and other electricity for each household. Note the large variation in average demand, which can also be seen in the boxplot shown in Figure \@ref(fig:demandBoxplot).
```{r means1, echo=FALSE}
load(paste0(dFolder, "meansDT.Rda"))
```

```{r demandBoxplot, out.width='100%', fig.cap='Box plot of average demand for each household'}
...
_Test for stationarity here? may not be necessary to include with STL_
...
```{r bothElecPlot, out.width='100%', fig.cap="Hot water and other appliances' electricity use over one day"}
...
While Figure \@ref(fig:bothElecPlot) may not in itself be very compelling, we can make a more thorough exploration into the temporal relationship between hot water electricity demand and that of other appliances using the cross-covariance function.
...
Another method that may be used to explore any cyclical effects or trends of time series data is that of STL decomposition. STL is an acronym for 'seasonal and trend decomposition using loess', whereby loess (short for local regression) is a method for estimating nonlinear relationships. 
...
The STL method was developed by Cleveland, Cleveland, McRae, & Terpenning (1990)[@Cleveland1990].
...
This section outlines a broad introduction to the various models selected for use either implicitly or explicitly in this analysis. Refer to Cowpertwait and Metcalfe[@Cowpertwait2009] for further information.
...
Preliminary modelling indicated that the seasonal ARIMA model was extremely computationally expensive, taking up to 8 hours to fit a single household. While this may be due to failings of a package-specific fitting algorithm, this model was not pursued any further.
