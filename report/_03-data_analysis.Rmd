...
meansDT %>%
  knitr::kable(caption = "Mean values of electricity demand for hot water and other appliances")
```
...
A q-order moving average process, $MA(q)$ can be expressed as a linear combination of the current white noise residual ($w_t$, see \@ref(TSoverview)) and the $q$ most recent previous residuals, defined as
\begin{equation}
...
This method decomposes the data into seasonal, trend and remainder components, (refer to Section \@ref(STL)), and then fits an ARIMAX model to the remainder using lagged values of other appliance electricity demand as the external regressors (refer to Section \@ref(ARIMAX)). This is then recombined with the seasonal and trend components to complete the model.
...
As demand respose is most crucial during daily peak periods, additional analysis is carried out to acertain the accuracy during grid peaks (defined as 7am to 9am, and 5pm to 8pm[@TranspowerNZ2015]).
