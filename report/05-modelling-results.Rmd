...
The cyclic nature of hot water demand would suggest that the seasonal ARIMA model would be well suited for our purposes. In addition, as this model falls under the 'conventional' forecasting catagory, it would be reasonable to assume it was computationally efficient. However, preliminary modelling found that the seasonal ARIMA model was extremely computationally expensive. Households were taking upwards of 8 hours each to fit this model to their data. While this may be due to failings of a package-specific fitting algorithm, the seasonal ARIMA model was not pursued any further.
