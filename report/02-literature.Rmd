...
_Michaels comment: "I still feel like I do not have a good feel for how prediction of electricty demand has been used or the finding of studies that have tried to predict demand yet. For example, if these studies have developed methods (an applied them in similar area) that give very accurate predictions and cover all of your metrics why would you need to do this study? If that is not the case then where do they fall short?"_
...
Research comparing AI methods showed SVMs tend to outperform ANNs in accuracy[@Ahmad2014] while having reasonable computational efficiency[@Ahmad2014; @Magoules2016], whereas ANNs in particular were noted as being computationally expensive[@Dervilis2018].
...
To the best of my knowledge, there is no existing literature that compares a range of models to forecast residential hot water _electricity_ demand.
...
When computational costs are mentioned, they are qualitative, and never given much focus. 
There is very little consideration within existing literature to utilising parameters from forecasting models in order to simulate demand within a physical model.
...
These researchers also compare a selection of forecasting methods to predict _volumetric_ residential hot water demand.
