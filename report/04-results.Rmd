...
The random walk model has RMSE ranging between `r round(min(dfSummary$RMSE[dfSummary$model == "naive"]), 2)`W and `r round(max(dfSummary$RMSE[dfSummary$model == "naive"]), 2)`W, with the average being `r round(DFsummary$RMSE[DFsummary$model == "naive"], 2)`W.
...
```
Figure \@ref(fig:simpleLinear) shows the effectiveness of this model for four seperate households. 
<!--
...
Parameters are selected in order to minimise the AIC (Equation \@ref(eq:AIC)) are provided in the Appendix . For to restrict computational expense in model fitting, maximum values for $p$, $d$ and $q$ are fixed at 5, 2 and 5, respectively.
