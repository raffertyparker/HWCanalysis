...
Temporal variation in electricity demand occurs at a variety of timescales in an electricity grid. As electrical energy cannot be stored, electricity supply must match this fluctuating demand. Providing electricity at times of very high demand requires the building and maintenance of underutilised generation and transmission infrastructure, a costly and inefficient situation[@Strbac2008]. 
...
The grid benefits in being able to rapidly increase generation or decrease load is reflected by the ability to monetize this capacity through various electricity markets.
...
In the event of a large generator unexpectedly going offline, a high voltage direct current (HVDC) pole tripping, or other large disruptions to supply there must be reserve electricity available so that the frequency is maintained within an acceptable range (between 48 and 52 Hz). This is provided by the instantaneous reserves market[@EANZ2014]. The systems operator will pay a premium for any electricity available to be generated during these periods (know as generating capacity), or load able to be shed during these periods (known as interruptible load).
...
Due to their ability to store large amounts of energy in the form of heat, hot water cylinders are generally the most “flexible” appliances in New Zealand households[@Stephenson2017], rendering them perfect candidates for demand shifting.
...
Simulations by Denis, Suard, Lomet and Chèze[@Denis2019] demonstrated average energy savings of up to 10% in hot water systems that only heated according to what was required.
...
To facilitate reproducibility of results, ANN methods were therefore not included within this research.
...
Support vector machines may be used in electricity forecasting by automating the process of discovering, categorising and quantifying various relationships within the data, such as those previously described as part of time series analysis and causal modelling[@Wei2018].
This method is less computationally expensive to train than ANNs, and better suited to solving non-linear problems such as those encountered in cyclic time series data[@Ahmad2014]. 
<!--
...
O'Shaghnessy, Cutler, Ardani and Margolis (2018)[@OShaughnessy2018] used the U.S. National Renewable Energy Laboratory's Renewable Energy Optimisation (REOpt) and Building Energy Optimisation (BEOpt) models to minimise the lifecycle cost of energy for a household through optimal configuration of PV, battery storage, smart domestic water heater, and smart AC unit. The household thermal storage devices (electric water heater, AC unit) were controlled through adjustment of the thermostat set-points through the day to take advantage of the daytime PV output. It was found that under current pricing, load-shifting through the use of smart AC units and smart domestic water heaters is a more cost effective method of increasing self-consumption of solar PV than the installation of batteries.
...
Cui, Wang, Wang, and Zhang[@Cui2016] (mentioned in previous section) use some variation or ARIMA modelling for their short term electricity demand forecasting at individual household level.

After confirming the importance of accurate DHW forecasts for effective demand response, Lomet, Suard, and Chèze [@Lomet2015] propose the use of an ARMA model fitted to household level volumetric DHW consumption data for this purpose.  

Aki, Wakui and Yokoyama[@Aki2016] also use volumetric DHW data, however they opt for a variation of SVM known as support vector regression as the forecasting method, and use a 'ground-up' approach of separating the data into estimated end-uses, such as bathing, kitchen etc.

Popescu and Serban[@Popescu2016] use an ARMA model for forecasting day-ahead hot water consumption of a block of 60 apartments under a district heating system.

Denis, Suard, Lomet, and Chèze[@Denis2019] considers the ability of forecasting to assist in reducing thermal losses from DHW tanks by controlling the operation of the device such that only the necessary amount of hot water is stored in the tank at once. An ARIMA model is utilised along with the TRNSYS energy simulation software. Overall energy reductions of up to 17.4% were demonstrated under this control scenario.

Gajowniczeka and Ząbkowski (2014)[@Gajowniczek2014] compare ANN and SVM forecasting methods to forecast aggregate household demand over 24 hour periods. Both models displayed similar performance in accuracy, however no consideration was given for other comparative metrics.

Ahmad, Hassan, Abdullah, Rahman and Hussin (2014)[@Ahmad2014] also compared ANNs and SVMs, as well as some conventional forecasting techniques, for building-level short term electricity demand forecasting. They concluded that the AI methods outperformed conventional methods in accuracy, with SVM models generally outperforming ANNs. No comparison on computational expense was provided.

De Felice, Alessandri, Ruti (2013)[@DeFelice2013] used weather forecasts as an external regressor in an ARIMAX model to predict Italian grid-level electricity demand up to 5 days in advance. This was compared against a naive model and an ARIMA model, with the inclusion of weather data into the models being found to significantly increasing the accuracy of prediction.

Jain and Satish (2009)[@Jain2009] used SVMs to forecast day-ahead grid-level electricity demand. While ANNs were mentioned for their prevalence in electricity demand forecasting, SVMs were ultimately chosen as the forecast method for their generalisation, performance, and automatic model parameter selection capabilities.
