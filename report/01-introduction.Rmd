...
Smart control simulations have demonstrated energy savings of up to 10% in hot water systems that only heated according to what was predicted to be required. These savings could be increased up to 17% with perfectly accurate predictions[@Denis2019].
...
There are a number of techniques commonly used for predicting hot water electricity demand. These may be broadly separated into the catagories (i) physical modelling, (ii) conventional forecasting, and (iii) artificial intelligence (AI)[@MatDaut2017].

### Physical modelling

_This whole section needs editing to fit where it now sits._

Smartly controlling residential hot water cylinders is an active research topic[CITE]. Much of this research uses computer models that replicate a hot water cylinder and its operation under standard residential hot water demand patterns.
These demand patterns must be input as parameters into the computer model.
In some cases, the parameters used to specify demand for these types of simulations are interchangeable with those we would obtain from a forecast of hot water electricity demand.
Even when this is not strictly the case, the insights we can gain from models which predict hot water electricity demand are applicable when building models to replicate hot water demand.
This suitability of forecasting parameters for inputs into demand simulations for research purposes is examined in more detail in Chapter \@ref(lit-rev).
...
Although the distinction is subtle, time series forecasting methods may be broadly separated by their computational efficiencies into two categories, AI and conventional.
...
Conventional forecasting methods involve discovering underlying statistical properties of data, which are then used to make predictions of the future.
These methods often require significant human input, generally requiring an initial data analysis, testing, and trial and error. However, once an optimal conventional model is discovered, it can generally be constructed with reasonable computational efficiency. 
...
This method, sometimes refered to as 'decomposition', can be acheived using a technique called 'seasonal decomposition of time series by Loess' (STL), and is explained in more detail in Section \@ref(STL).
...
Two AI models commonly used in electricity demand forecasting, namely support vector machines (SVMs) and artificial neural networks (ANNs), are explained in more detail in Section \@ref(IntroToModels).
...
The research conducted for this thesis uses existing electricity demand time series data that had been previously collected from New Zealand households[@R-GREENGridData].
The aim is to use this data to develop a computationally efficient, accurate, and physically meaningful method of predicting individual household hot water electricity demand at 30 minute resolution. 
...
Chapter \@ref(discussion) provides comparisons of the models based on the metrics outlined in Chapter \@ref(methodology), with optimal models for different purposes presented.
A summary of the background and aspirations of this thesis and its results are provided in the conclusion (Chapter \@ref(conclusion)). Potential further work is also detailed in this section.
Relevant tables, including model parameters and some data attributes, are provided in Appendix \.
