...
The reason why this is a worthwhile thing to be doing, put simply, is that it has the potential to increase efficiencies.
...
A large tank of water being maintained at 65°C day and night, while only a portion of it is being used at particular times of day or week for the vast majority of the time, is thermally inefficient.
An electricity grid that is built and maintained to generate and transmit enough electricity to cover periods of extremely high demand, while only a portion of this capacity is being used for the vast majority of the time, is economically inefficient.

This chapter explains these two inefficiencies in more detail and outlines the role that smarter control of hot water cylinders may play in alleviating these inefficiencies. It then goes on to explain the value that predictive modelling would bring to such a smart control system.
<!--
...
```
## The New Zealand electricity grid
...
These markets were created in 1996 following privatisation of electricity grid.
...
They use this system both to defer upgrades to their infrustructure that would otherwise be necessary, and also as a form of revenue, by selling into the reserves markets previously mentioned.
...
An important (but again, not clear-cut) distinction between these two methods is that of computational efficiency. 
Computational efficiency can be thought of as the amount of computing energy necessary to process and analyse the data in the construction of the forecast. This is most inutively understood as the amount of time a computer with a particular processing capability will take to construct the forecast. 
Conventional methods often require significant human input, generally requiring an initial data analysis, testing, and trial and error, but once the optimal model is discovered, it tends to be very computationally efficient.
AI methods tend to require less human time to build the forecast, while requiring significantly more computational time.

### Conventional forecasting methods

Conventional forecasting may again be separated into two distinct techniques.
...
The aim of this project is use electricity demand data from New Zealand households to develop an efficient, accurate, and physically meaningful method of predicting individual household hot water electricity demand using time series data at 30 minute resolution. 
