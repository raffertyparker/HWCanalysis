...
The first is that of unnecessary or premature capital expenditure on electricity networks.
...
The second is that of heat losses from residential hot water cylinders. Many households constantly maintain a large tank of water at around 65°C, while only requiring a portion of it at particular times of day or week. This is an example of energy inefficiency.
...
The data used to construct and test these models was collected from households in New Zealands part of a previous study[@GREENGrid].
New Zealand has a high percentage of both electric hot water cylinders and households with smart electricity meters, and therefore would be an ideal candidate for the kind of smart control system these predictive models would improve.
As such, the introduction is presented from a New Zealand context.
However, these methods prperly implimented would improve economic efficiencies of any electricity network with a high proportion of electric hot water cylinders, and energy efficiencies of hot water cylinders in general.
...
These average increases are more pronounced at certain times of day, generally reflected as two 'peaks' in the morning and evening. This is due to the effects of high residential electricity demand during these periods.
...
In New Zealand, each additional kW of this peak-time demand is estimated to cost $187 per year in additional costs[@Jack2016].
...
Electricity generators submit bids on a spot market to supply electricity to each node at a particular cost during each half-hour period. These are then aggregated to form a supply curve for each node.
...
Price variations within these markets occur according to supply and demand. Prices rise when there is high electricity demand on the grid or there is limited availability of electricity resources with low marginal cost of production, such as those from renewable sources. Conversely, prices fall when electricity is cheap to produce, or total electricity demand is low. 
...
Local network companies (also known as electricity distribution businesses or 'EDBs') are in charge of transmitting electricity from the grid nodes to individual households, and charge retailers a fixed charge per kWh of electricity delivered for the service[@Stephenson2018].
...
However, in New Zealand there has been significant efforts by electricity retailers and distribution businesses to deploy smart-meters to residential electricity consumers. 
...
Large electricity users are often provided a discounted kWh price of electricity, while being charged high penalties for any electricity consumed during times where there is very high local network demand (known as 'congestion periods')[@Orion2017].
...
Once again, this provides market value to the ability to shift electricity demand in time. These demand charges can make up a significant portion of the electricity bill of a large consumer, and are a primary source of revenue for the systems operator[@Nair2008]. Currently, residential consumers are not directly exposed to these charges. Instead, the charges are 'bundled in' as a flat rate per kWh lines charge in the electricity bill. 
...
Under a ripple controlled system, local electricity distribution network companies provide a discounted electricity rate for appliances that are separately metered, under the condition that they are able to remotely cease the supply of electricity during peak periods or grid emergencies[@Orion2017]. 
...
There is a way this may be obtained using technology already deployed in a majority of New Zealand households, namely, smart meters.
Many household already have smart meters that record hot water electricity demand separately to other electricity demand, in order to conduct ripple control services.
This provides the physical capability to temporally shift hot water electricity demand by suppressing the element though the ripple control signal.
...
Flexible devices are obvious candidates for autonomous control objectives, as they have significant potential to temporally shift their demand for electricity with minimal impact to the end user (referred to as 'impacts on service').
...
Due to the high flexibility of residential appliances such as hot water cylinders, smart control technology could potentially enable them to provide significant demand shifting[@Dhulst2015].
...
Smart control of residential hot water cylinders is thus particularly promising for demand response, as it is both flexible, and a key contributor to network demand peaks[@Stephenson2017].
...
Smart control simulations have demonstrated energy savings of up to 12% in hot water systems that only heated according to what was predicted to be required[@Kepplinger2015]. These savings could be increased up to 17% with perfectly accurate predictions[@Denis2019].
...
Different techniques use different algorithms to carry out the forecasting processes, some of which use more computational resources than others.
...
It is not strictly necessary to predict electricity demand in order to smartly control electricity using appliances.
However, accurate demand predictions would allow devices to be controlled according to the usage patterns of individual houses[@Lomet2015]. 
...
It would also improve the ability to participate in half-hour ahead demand response markets. 
Demand predictions could allow more accurate bids as to an estimated capacity for load-shedding. 
...
Techniques commonly used for predicting hot water electricity demand may be broadly separated into the catagories (i) physical modelling, (ii) conventional forecasting, and (iii) artificial intelligence (AI)[@MatDaut2017].

### Physical modelling

Smartly controlling residential hot water cylinders is an active research topic[@Jack2018; @Roux2018; @Roux2017; @Sepulveda2010; @Gelazanskas2016]. 
Much of this research uses computer models that mathematically replicate a hot water cylinder and its operation under standard residential hot water demand patterns.
...
These demand patterns are often based on simple statistical or mathematatical relationships related to time, with some artificially generated 'randomness' for added realism.
Uncovering underlying patterns within data that can be represented as simple statistical or mathematical relationships is the primary focus of conventional forecasting methods.
...
In addition, the insights that are gained from from conventional forecasting methods may be applicable to physical models.
...
This method, sometimes refered to as 'decomposition', can be acheived using a technique called 'seasonal decomposition of time series by Loess' (STL), and is explained in more detail in Section \@ref(STLmeth).
...
These methods can be considered separate from their conventional counterparts by their ability to process and establish relationships between data autonomously.
This allows them to be constructed with minimal human input or preexisting insight into features of the data.
This is done through a method known as 'training'. To train an AI model, an algorithm iteratively adjusts parameters within the model such that the outputs (predictions) of the model best match the actual data, under certain prespecified constraints.
...
The ability to accurately predict the demand of flexible appliances such as hot water cylinders increases the viability of smartly controlling them.
Increasing the viability of smart control technology hastens its uptake. This in turn assists in keeping electricity cheap and plentiful, which is crucial in the drive to electrify carbon-intensive industries.
...
The aim is to use this data to develop an accurate, interpretable, physically meaningful and quick to compute method of predicting individual household hot water electricity demand at 30 minute resolution. 
...
This chapter introduced two key inefficiencies that forecasting of hot water electricity demand could help alleviate. It also provided a brief introduction to the type of data and techniques used to make these forecasts. Finally, it outlined the aims of this research.
The following chapter presents relevant literature. It highlights key research in relevant topics, in particular, hot water modelling, smart control of appliances, and electricity demand forecasting.
Chapter \@ref(methodology) presents my methodology. It introduces the electricity demand dataset utilised in this work, and explains the data cleaning and preparatory process. It then describes the exploratory data analysis techniques and the forecasting models utilised, and the metrics by which the models are compared to one another.
Chapter \@ref(prelimDA) shows the results obtained from the preliminary data analysis methods. It also briefly discusses the implications these results have for selecting appropriate forecasting models.
...
A summary of the background and aspirations of this thesis and its results are provide along with potential further work in the conclusion (Chapter \@ref(conclusion)).
Relevant code and tables, including model parameters and some data attributes, are provided in the Appendix.
