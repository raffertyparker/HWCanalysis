...
Having a large tank of water being maintained at 65°C day and night, while only a portion of it is being used at particular times of day or week for the vast majority of the time, is thermally inefficient.
...
Interruptible load that is able to be shed within sixty seconds and remain reduced util instructed otherwise by the systems operator is eligible to be traded in the sustained instantaneous reserves (SIR) market.
...
Congestion period demand charges are another market-based mechanism for adjusting demand. 
...
Another demand-shifting mechanism is that of ripple control. 
...
There have been proposals[@NewZealandElectricityCommission2009] that a market-based tactic to assist demand resonse efforts would be to encourage residential consumers to move to a real-time pricing plan that uses smart meters to send price signals in order to disincentivise peak-time electricity usage. 
...
With the rapid rise of 'internet of things' technology, whereby devices are interconnected and offer the ability to be controlled remotely, there is the ability to automatically schedule flexible devices. 
Devices may be scheduled around availability of renewable energy sources, or optimised to minimise costs through participation in demand response markets.
...
The smart grid is a power grid where energy producers, network companies and consumers are intelligently connected to each other. On the consumer side this largely enabled by advanced electricity meters, known as 'smart meters', which have the ability to store, display and transmit data associated with the electricity being consumed. 
This two-way information communication is proposed to encourage the integration of distributed (generally renewable) power generation, and allow sophisticated metering and automation to proliferate\cite{Lund2015}. 
The process of autonomously scheduling appliances for efficiency benefits is referred to as 'smart control'
Due to the high flexibility of various residential appliances, smart control technology could thus enable significant demand shifting\cite{Dhulst2015}.
...
Secondly, computer models provide the ability to ‘speed up’ the passing of time, and to be run ‘into the future’, two features currently unavailable when conducting real-world analysis. 
...
The model in [@Heleno2015] simply schedules three 35L, 38$^\circ$ hot water events at three separate times through the day.
The model in [@JackDew2018] simulates domestic hot water usage by assuming up to three major use events (representing showers, bathing etc) that are random and normally distributed about fixed times of day, with additional minor usage events (representing hand washing etc) having equal probability of occuring between specified intervals at other times.
...
These scenarios generally involve simulating the suppression of the element or adjustment of thermostat at key times in order to reduce grid peaks[@Roux2018; @Sepulveda2010, @Kepplinger2015], provide grid-beneficial (instantaneous response) services[@Kondoh2011], minimise costs under time-varying electricity prices[@Kepplinger2015; @Mohsenian-Rad2010; @OShaughnessy2018; @Sepulveda2010; @Cui2016; @Kapsalis2018], utilise low-carbon electricity[@Ramchurn2012], and minimise ‘cold water events’ under a budget constraint[@Yu2013].
...
_Include more forecasting papers_
