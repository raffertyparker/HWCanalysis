...
However, it is important that the models constructed in this thesis are able to predict these values, as they indicate times where the element is reheating the water after a usage event.
...
The three most significant frequencies did not include a weekly period for any households.
...
As ARMA models (see Section \@ref(ARIMAmethodology)) are more accurate for stationary time series', hot water electricity data was tested for stationarity and differencing was performed (see Section \@ref(diff)) if data was non-stationary.
...
Figure \@ref(fig:crossCovariance) shows that our hot water electricity use is positively correlated with other electricity use over small time lags. 
...
For half-hour ahead predictions, correlations with lags of at least half an hour are necessasry.
Some households appear to have their maximum correlation at half hour lags, and all others still show positive correlation for this lag.
...
The maximum correlation between hot water electricity demand and lagged other electricity demand occurs within the first two lags for most households. Thus, when incorporating other appliance demand into forecasting models, we use the previous two lags to best capture this correlation. 
