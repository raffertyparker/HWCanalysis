...
Data processing code can be viewed in Appendix. 
...
We would expect residential hot water electricity demand to follow total residential electricity demand in being somewhat cyclic, with daily and weekly periodicity due to the routines of the household occupants. These cycles are referred to as 'seasonality'. This terminology is due to the historical reason that time series methods were often developed to analyse econometric and financial data which often vary with the seasons, for example an increase in house sales during summer months[@Wooldridge2009]. There are a number of different methods which allow us to explore seasonality within data. One method is autocovariance. Autocovariance compares the covariance of a stochastic process (such as our time series data of hot water electricity use) with itself at different time steps. This is a valuable tool for visualising and quantifying cyclical behaviour of data, and is defined as
...
Let $X$ be a random variable with a finite number of finite outcomes $x_1, x_2, \ldots, x_k$ occurring with probabilities $p_1, p_2, \ldots, p_k,$ respectively. The expected value of $X$ is defined as
...
The results of the autocovariance function are interpreted by plotting $\gamma_k$ against the lag values. _cite package_
...
It achieves this by representing a set of time series data in terms of sinusoidal functions[Bloomfield2000].
While a description of the precise mechanics of how this is acheived is beyond the scope of this thesis, the general idea is as follows.
...
Naive models used in this research were created using the `naive` function within the `forecast` package[@R-forecast].
...
Autoregression and moving average processes may then be considered individually as special cases of an ARIMA process, $ARIMA(p,d,0)$ and $ARIMA(0,d,q)$ respectively.
...
While ANNs are commonly used in electricity demand forecasting, literature suggests they tend to be outperformed in both accuracy and computational efficiency by support vector machines for this type of analysis. 
For this reason, I opted not to consider them in this comparative work, instead choosing support vector machines as my comparative AI method.
