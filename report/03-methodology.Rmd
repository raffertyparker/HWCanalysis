...
The dataset is publicly available from the UK Data Service. Publications to date that have utilised the dataset include Refs. [@Ocampo2015], [@Suomalainen2017], [@Stephenson2018], [@Jack2018], and [@JackDew2018].
...
In order to succinctly describe time series analysis methods it is useful to utilise the backshift operator.
...
There are a number of different methods which allow us to explore seasonality within a time series. One method is autocovariance. Autocovariance compares the covariance of a stochastic process (such as our time series data of hot water electricity use) with itself at different time steps. This is a valuable tool for visualising and quantifying cyclical behaviour of data, and is defined as
...
Its description is therefore beyond the scope of this thesis, but can be examined in Ref. [@Cleveland1990]. 
For our analysis, STL decomposition was conducted using the R packages `stats`[@R-base] and `forecast`[@R-forecast].
Decomposition of data using STL can be a highly effective addition to a hot water demand model, as documented in Ref. [@Gelazanskas2015].
...
Figure \@ref(fig:STLPlot) illustrates an STL decomposition of a household over one week. The top panel, labelled 'observed', is the actual data, while the lower three panels display the trend, seasonal, and random components of this data. Note the repeating daily pattern in the seasonal panel.
...
Rather than fitting polynomial functions to local subsets of data as performed by STL, a Fourier analysis represents data in terms of a linear combination of sinusoids[@Bloomfield2000].

When applied to a collection of $n$ observations, $\{x_1, x_2, \dots, x_n\}$ this linear combination of sinusoids may be described mathematically as
\begin{equation}
...
The mechanics of how the parameters of Equation \@ref(eq:Fourier) are adjusted to best fit the data are beyond the scope of this thesis. For more information, refer to Refs. [@Brockwell1991] or [@Weron2006].
...
In a similar fashion to the autocovariance function given by Equation \@ref(eq:ACV), the cross-covariance function of two variables $x, y$ is defined by
...
Due to the range of metrics by which models are compared by (see \@ref(metrics)), some relatively simplistic models were included for comparison.
While these simple models may not perform as well as more complex ones in terms of accuracy, they have the potential to perform higher in other comparative metrics.
...
Previous values of these white noise residuals are used to predict future values of the dependant variable in moving average models.
...
Further details regarding this model can be found in Ref. [@Cowpertwait2009]. 
...
However, the seasonal naive model makes the assumption that for data that displays seasonality, the most likely value of the the next time step is that of the same time one period prior, i.e.
...
Further details regarding simple linear regression can be found in Ref. [@Cowpertwait2009]. 

### Differencing {#diff}

Differencing a time series $\{x_t\}$ is a simple method of removing trends in order to make the time series stationary (see Section \@ref(Statmeth)). This is often used in combination with other methods which become more more accurate when applied to stationary data.
...
An example of first order integrated time series data is demonstrated in Figure \@ref(fig:integratedExample), whereby total household electricity data becomes stationary after it is differenced.

(ref:diff) Total electricity demand data of household `rf_22` before and after first-order differencing

```{r integratedExample, echo=FALSE, fig.cap="(ref:diff)"}
...
The R function `auto.arima`[@R-forecast] was used to create ARIMA models. This function automatically select the parameters $p,d,q$ which minimise the AIC specific to the particular data being modelled. This is done iteratively according to the algorithm outlined in Ref. [@RobJ.Hyndman2008]. 
...
Further details regarding ARIMA models can be found in Ref. [@Cowpertwait2009]. 
...
This method was the most accurate model considered in Ref. [@Gelazanskas2015].
...
This method decomposes the data into seasonal, trend and remainder components, (refer to Section \@ref(STLmeth)), and then fits an ARIMAX model (Equation \@ref(eq:ARIMAX)) to the remainder component using lagged values of other appliance electricity demand as external regressors (refer to Section \@ref(ARIMAXmeth)). This is then added back to the seasonal and trend components to complete the model.
...
Figure \@ref(fig:kernelTrick) shows how, by transforming data from a two dimensional space into a three dimensional space using an appropriate function, the data may be linearly separated, in this case by a plane. 
...
Parameters $b$ and $\alpha_i$ in Equation \@ref(eq:Gamma) are adjusted by solving a quadratic optimisation problem. This is outside the scope of this thesis, but may be found in Ref. [@Fan2005].
...
For further details regarding using SVM models for forecasting electricity demand refer to Ref. [@Magoules2016], or for a more general overview, refer to Ref. [@Smola1999].
Support vector machines were created using the `svm` function[@e1071]. This function uses the training algorithms detailed in Ref. [@Fan2005].
...
Following from the definition of a white noise series provided in Section \@ref(WNmeth), and the autocorrelation function in Equation \@ref(eq:ACF), a white noise series has an autocorrelation function $\rho_k$ such that
...
Instead, the plot of $\rho_k$ should start at one and decay rapidly below a 5% significance level (shown on the plots as a blue dotted line).
...
'Black box' models and those that are comprised of a large number of parameters score poorly.
...
This chapter provided mathematical details of the methods and models that were used in this thesis based on their suitability according to existing literature.
