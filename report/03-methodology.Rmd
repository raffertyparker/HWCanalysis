...
## Correllation with other appliance electricity demand {#CCVmeth}
...
\begin{equation}
  ...
  (\#eq:ccv)
\end{equation}
where variable $x$ lags variable $y$ by lag $k$, and $\mu_x, \mu_y$ are the means of $\{x_t, x_{t+k}\}$, and $\{y_t, y_{t+k}\}$, respectively.[@Cowpertwait2009].
...
While these simple models may not perform as well as more complex ones in terms of accuracy, they have the potential to perform higher in the other comparative metrics.
...
As described in Section \@ref(TSoverview), white noise time series values are inherently unpredictable with an expected value of zero.
...
Differencing a time series $\{x_t\}$ is a simple method of removing trends in order to make time series data stationary (see Section \@ref(Statmeth)). This is often used in combination with other methods which become more more accurate when applied to stationary data.
...
```{r integratedExample, echo=FALSE, fig.cap="Total electricity demand data before and after first-order differencing"}
...
Inputs for maximum values of $p, d, q$ are necessary in order to bound processing time. 
...
As demand response is most crucial during daily peak periods, additional analysis is carried out to ascertain the accuracy during grid peaks (defined as 
@TranspowerNZ2015]). This was obtained by calculating the RMSE for all predictions that occurred during peak periods (denoted $RMSE_{peak}$). To assist comparisons, this is then used to calculate a percentage error increase (PEI) between the average RMSE and the average RMSE during peak times. The PEI is given by:
