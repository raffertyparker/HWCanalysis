...
The results of the autocovariance function are interpreted by plotting $\zeta_k$ against the lag values. This was obtained using the `Acf` function from the `forecast` package[@R-forecast].
...
-->
Support vector machines classify data samples by mapping them in hyperspace, separating them by a hyperplane, and distinguishing between them according to which side of the hyperplane they fall on. 
Essentially, this process allows a linear regression to be performed on data that may be highly nonlinear[@Smola1999].
...
When the data are not easily separated by a hyperplane in their current form, they may be transformed by a kernel function into a format that is more easily separable.
A general introduction to SVMs can be found in [@James2017], an overview of SVMs for time series forecasting may be found in [@Smola1999], and practical information regarding SVM use for energy analysis can be found in [@Magoules2016].
The SVMs used in this research were built in the following manner.

The maximum margin hyperplane is denoted as $\Gamma$, which is defined mathematically in terms of the variables within our data as
...
For simplification, we denote $\textbf{z}_t$ to be the vector comprised of our data variables, $\textbf{z}_t := \{h_t, x_{t-1}, x_{t-2}, y_{t-1}, y_{t-2} \}$.
...
Following this, predictions for our hot water value $x_t$ can be obtained by inputting corresponding $\textbf z_t$ vector values of $h_t, x_{t-1}, x_{t-2}, y_{t-1}, y_{t-2}$ into Equation \@ref(eq:GammaOrig). In keeping with the notation of the rest of this chapter, we may then denote out model as
...
This is an example of 'overfitting' a model, whereby a model goes beyond capturing key statistical properties of the underlying data and begins fitting itself to the random fluctuations about these properties, the unpredictable white noise component.
...
All models within this research were built and tested in this manner, with the exception of the two naive models.
...
For this work, model accuracy is determined by the root mean square of the residuals, with lower values indicating higher accuracy. This is referred to as the 'root mean square error (RMSE). 
